...
public class UltraFast implements UltraStack {
  ...
  public void display() {
    ...
    for (int i = 0; i < levels; i++) {
      ...
      int spaces = (int) Math.pow(2, levels - i - 1) - 1;
      ...
      for (int j = 0; j < nodes; j++) {
        ...
        if (index < maxRows.length) {
          System.out.print(String.format("%2d", maxRows[index]));
  ...
  public static void visualizeKSum(UltraFast stack, int k) {
    stack.display();

    System.out.println("\nVisualization of ksum(" + k + "):");
    drawTree(stack, 0, "", true);

    int currentIndex = 0;
    long sum = 0;
    int numZeros = (int) Math.pow(2, stack.height - 1) - stack.n;
    k += numZeros;

    int nodesOnRight = (int) Math.pow(2, stack.height - 2);

    System.out.println("\nInitial State: currentIndex = " + currentIndex + ", k = " + k + ", sum = " + sum);

    while (k > 0) {
      if (k >= nodesOnRight) {
        k -= nodesOnRight;
        sum += stack.sumRows[currentIndex * 2 + 2];
      ...
      System.out.println("State: currentIndex = " + currentIndex + ", k = " + k + ", sum = " + sum);
    }

    System.out.println("\nResult of ksum(" + k + "): " + sum);
  }
